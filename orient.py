...
if model.lower() == "nearest":
    ...
    if switch.lower() == "test":
        ...
        trainVector = np.zeros((numLinesTrain + 1, 192), dtype=np.int_)
        trainOrient = np.zeros((numLinesTrain + 1, 1), dtype=np.int_)
        testVector = np.zeros((numLinesTest + 1, 192), dtype=np.int_)
        testOrient = np.zeros((numLinesTest + 1, 1), dtype=np.int_)
        lineNumber = 0
